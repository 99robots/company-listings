...
if($_GET['task'] == "doedit") {
  ...
  $type = $_GET['type'];
  $address = str_replace( "'", "\\'", str_replace( "\\", "\\\\", $_GET['address'] ) );
  $uri = $_GET['uri'];
  $description = str_replace( "'", "\\'", str_replace( "\\", "\\\\", $_GET['description'] ) );
  $owner_name = str_replace( "'", "\\'", str_replace( "\\", "\\\\", $_GET['owner_name'] ) );
...
<form id="admin" class="form-horizontal" method="GET">
  ...
  <fieldset>
    ...
    <div class="form-actions">
      ...
      <a href="<?php echo get_permalink(); ?>" class="btn" style="float: right;">Cancel</a>
    </div>
  </fieldset>
</form>
