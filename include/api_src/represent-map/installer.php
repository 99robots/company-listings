...
if(isset($_POST['installer_submitted'])) {
	...
	if(!$error) {
		$file = plugin_dir_path( __FILE__ ) . 'include/db.php';
	...
	if(!$error) {
		$config_file_contents = file_get_contents(plugin_dir_path( __FILE__ ) . 'include/db_example.php');
		...
		<div id="content">
			<form class="well form-inline" action="" id="login" method="post">
				...
				<? else : ?>
					...
					<? $dirperms = substr(sprintf('%o', fileperms(plugin_dir_path( __FILE__ ) . 'include')), -4);?>
					<? if($dirperms != '0777' && $dirperms != '0755' && !chmod(plugin_dir_path( __FILE__ ) . "include", 0755)) : ?>
